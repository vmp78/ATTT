...
# XORs two strings
def strxor(a, b):  # xor two byte strings (trims the longer input)
    ...
    for index, ciphertext2 in enumerate(ciphers):
        if current_index != index:  # don't xor a ciphertext with itself
            for indexOfChar, char in enumerate(
                    strxor(bytes.fromhex(ciphertext), bytes.fromhex(ciphertext2))):  # Xor the two ciphertexts
                # If a character in the xored result is an alphanumeric character, it means there was probably a space character in one of the plaintexts (we don't know which one)
...
# We then confirm this is correct by producing the key from this, and decrypting all the other messages to ensure they make grammatical sense
target_plaintext = "The secret message is: When using a stream cipher, never use the key more than once"
print(target_plaintext)
key = strxor(bytes.fromhex(target_cipher), target_plaintext.encode())
for cipher in ciphers:
    print(strxor(bytes.fromhex(cipher), key).decode())
